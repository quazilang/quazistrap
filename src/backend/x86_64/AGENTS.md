...
- Checks `__void_trace_enabled`; if set, calls `__void_print_backtrace` (RBP chain walk, up to 16 frames)
- If not set, prints `use VOID_TRACE=1 to see full stack trace\n`

---

## Panic Handler

`PanicInfo` carries `message`, `file`, `line`. `__void_panic_handler` prints all three, then calls `__void_print_backtrace()` (intrinsic ID 25) before exiting 101.

`panic("msg")` call sites get `file`/`line` injected by codegen. `prelude/array.void` uses `panic("index out of bounds")` for `Array.get` / `Array.set` / `Index` OOB checks.

---

## `@no_crash`

File-level directive (like `@no_std`). When present, the entry stub omits crash-handler registration (Linux `sigaction`, Windows `AddVectoredExceptionHandler`). `__void_print_backtrace` is still emitted so panic backtraces work. Produces a smaller binary (~1 KB smaller).

---

## `VOID_TRACE=1` Detection
...
- Both set the `__void_trace_enabled` byte in `.data`.
