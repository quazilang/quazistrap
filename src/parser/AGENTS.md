...
- **`else if` chains**: Currently parsed as `else { if cond { ... } }` (nested `If` inside `else_block`). A dedicated `else_if: Vec<(Expr, Block)>` field on `If` would flatten the AST and improve codegen. See P1 roadmap in `CLAUDE.md`.
