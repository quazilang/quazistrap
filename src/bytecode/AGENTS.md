...
- **`else if` chains**: Currently compiled as nested `If` inside `else_block` (two jumps per branch: `Jz` over then-block + `Jmp` over else-block). A flattened AST would allow chained `Jz` jumps with a single final `Jmp` to the end, saving one jump per `else if` branch. See P1 roadmap in `CLAUDE.md`.
