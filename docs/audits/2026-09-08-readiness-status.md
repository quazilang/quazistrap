...
- Superseded LSP document generations now interrupt tokenization, parsing,
  semantic pass boundaries, top-level declaration and reachable-statement
  walks, and long semantic collection/traversal loops. The compiler returns
  an operational cancellation result rather than publishing a partial report
  or source diagnostic. Individual expressions remain atomic.
...
| LSP | Workspace symbols, loader-backed definitions, and loader-backed cross-file references exist for local/package/std imports. Rename produces one atomic workspace edit only when every resolved occurrence is inside client-negotiated workspace roots. Compiler-backed inferred type hints are available for unannotated local declarations, and W03 single-selector imports offer a safe removal quick fix. The server explicitly negotiates open/close and save notifications with incremental changes. Compiler and workspace-index snapshots run on Tokio's blocking pool; loader snapshots are discarded if their included open buffers change or close, and save-time index writes are generation-checked. The transport honors pending-request cancellation. Superseded document diagnostics cooperatively stop in lexer/parser work, loader traversal and public-export resolution, semantic pass/top-level-item/reachable-statement boundaries, and long semantic collection/traversal loops; individual expressions remain atomic. Neovim 0.12 and VS Code 1.133 have isolated real-client hover smokes. | Real-client feature-request coverage for Zed and Helix. |
