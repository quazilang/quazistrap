...
| Editors | Local integration repositories exist. Neovim 0.12 has a headless real-client smoke that opens a `.qz` project, starts `qz lsp`, requests hover, and performs the shutdown/exit protocol path. VS Code 1.133 activates the development extension for a `.qz` file, starts `qz lsp`, observes server initialization, and shuts down cleanly. | A VS Code real-client feature-request smoke; versioned runtime validation for Zed and Helix. Zed and Helix runtimes are unavailable locally. |
