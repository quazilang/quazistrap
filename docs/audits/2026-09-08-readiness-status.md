...
- Superseded LSP document generations now interrupt tokenization, parsing,
  semantic pass boundaries, top-level declaration walks, and long semantic
  collection/traversal loops. The compiler returns an operational cancellation
  result rather than publishing a partial report or source diagnostic. The
  focused semantic and LSP tests pass.
