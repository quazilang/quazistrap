...
specific, lives under `target/`, and must not be committed.
...
net_git = { type = "git", url = "https://example.org/net.git", rev = "v1.2.0" }
...
Downloads are stored in the global Quazi package cache. Archives are extracted
...
dependencies are locked to a commit revision. Run `qz fetch` to materialize and
verify dependencies, and `qz deps` to inspect the result.
...
```text
target/quazi/<architecture>-<os>/default/incremental.qzc
```
...
`qz clean` removes the cache with `target`.
