...
is open. Both `raw_handle()` and `unsafe symbol(name)` use a shared receiver:
they may inspect an open library but cannot overlap `close()`. `DynamicLibraryError.message()` provides display text for diagnostics;
