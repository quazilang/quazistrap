...
storage). Phase 3 now has a narrow contiguous-container destruction foundation;
borrowed element APIs, replacement, and removal remain design work. This page turns maintainer decisions
...
#### Implemented phase-3 foundation: contiguous element destruction

`@contiguous_elements(element=T, pointer=ptr, length=len)` is a validated
aggregate contract for a generic, ordinary Quazi struct with initialized,
contiguous elements in `[0, len)`. Semantic analysis publishes the canonical
element-parameter index and pointer/length field names in `SemanticReport`;
code generation only translates those validated field names through the
semantic layout offsets. It does not identify `Array`, `Header`, or any library
API by name.

When an instance requires cleanup, generated bytecode counts backward from its
length, loads each concrete element with its resolved slot stride, applies the
...
containers with holes. The source `free` method is the storage-release hook;
it must not also walk elements, which would double-destroy them. The compiler
continues to reject by-value reads of owned Array elements. This phase therefore
establishes destruction only, not a `Headers` migration.
