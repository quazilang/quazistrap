...
- QZI v10 is a sectioned executable/library bytecode container with package metadata, a public source interface, named call relocations, a mandatory canonical ownership-artifact envelope, signedness-correct integer instruction flags, affine function-value ownership, layout-query and process-runtime intrinsic IDs, and legacy chunk payloads. The current envelope content-binds metadata, interface, relocations, and bytecode but is explicitly unverified: it is not a D-014 certificate and never permits a QZI-only borrowed safe call. Readers retain compatible QZI v2-v9 bytecode as `LegacyAbsent`; v1 omitted frame metadata, parameterized v6 trait interfaces lack receiver metadata, and pre-v7 public function-value contracts or synthetic closure/forwarder chunks require a source rebuild. Immutable golden fixtures from real historical writers (`src/bytecode/fixtures/qzi/`) lock the v2-v6 reading paths, including the v2 chunk-header layout without a flags byte and v3 string-based `@api` metadata. QZI libraries work without original source; generic template bodies remain source-only for now.
- QZC v8 (`build/quazi/<target>/<artifact>/incremental.qzc` by default) stores exact-hit linked QZI plus source-hashed pre-WPO function chunks. Partial misses restore unchanged chunks, compile changed files, then rerun full-program WPO. V8 invalidates v7 caches because QZI v10 adds the ownership-artifact envelope. Signature/type/import/config changes conservatively invalidate all chunks. Downloaded dependencies live in `<out_dir>/deps`.
...
| `@contiguous_elements(element=T, pointer=ptr, length=len)` | Compiler-validated physical contract for a dense generic container. Semantic analysis records its element parameter and named pointer/length fields; codegen translates only that semantic contract through resolved layout offsets. Generated cleanup destroys owned `[0, len)` elements before the consuming source `free(self)` storage-release hook. It is not an element-borrow or replacement API. |
